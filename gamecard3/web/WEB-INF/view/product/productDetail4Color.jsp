  ...
  <%@include file="head.jsp" %>
          ...
          <hr>    		
              ...
              <div class="col-lg-8">
	                ...
	                <c:if test="${!empty  colorList}">
	                 <div>
		                  <div class="col-lg-3"><p class="text-right"><strong>Color:</strong></p></div>
		                  <div class="col-lg-9"><p class="text-left"> 
		                  		<c:forEach items="${colorList }" var="r">
		                  			<button class="btn btn-small" type="button" onclick="javascript:selectColor('${r.key}','${r.price}');">${r.key}</button>
		                  		</c:forEach>
		                  </p>
	                  </div>
	                </div>
    ...
    var selectedColor = "" ;
   	function addToCart2(productId,unitPrice){
	   	 ...
	   	 if(${fn:length(colorList)>0}){
	   		if(selectedColorFlg == false){
		   		 alert("Please select color") ;
		   		 return ;
		   	 }
	   	 }
	   	unitPrice = $("#actualSellPrice").html();
	   	addToCart(productId,amount,unitPrice,selectedColor) ;
   	}
   	
   	function selectColor(color,price){
   		selectedColorFlg = true ;
   		selectedColor = color ;

	       			...
	       			<c:when test="${page.totalPage <=10}">
	       				<c:forEach var="i" begin="1" end="${page.totalPage}">
		       				<c:choose>
		       					...
		       					<c:otherwise>
		       						<a href="<%=basePath%>/product/searchBy?pageIndex=${i}">${i}</a>
		       					</c:otherwise>
		       				</c:choose>
	       				</c:forEach>
	       			</c:when>
	       				...
	       				<c:choose>
	       					<c:when test="${(page.currentPage+1)<=5 }">
	       						<c:choose>
	       							...
	       							<c:otherwise>
	       								<a href="<%=basePath%>/product/searchBy?pageIndex=${page.currentPage}">&lt;Prev</a>
	       							</c:otherwise>
	       						</c:choose>
		       					<span class="disabled">&lt;Prev</span>
								...
								<a href="<%=basePath%>/product/searchBy?pageIndex=6">6</a>
								<a href="<%=basePath%>/product/searchBy?pageIndex=7">7</a>...
								<a href="<%=basePath%>/product/searchBy?pageIndex=${page.totalPage-1}">${page.totalPage-1}</a>
								<a href="<%=basePath%>/product/searchBy?pageIndex=${page.totalPage}">${page.totalPage}</a>
								<a href="<%=basePath%>/product/searchBy?pageIndex=${(page.currentPage+1)+1}">Next&gt;</a>
	       					</c:when>
	       					
	       					<c:when test="${(page.currentPage+1)>(page.totalPage-5)}">
	       						<a href="<%=basePath%>/product/searchBy?pageIndex=${page.currentPage}">&lt;Prev</a>
	       						<a href="<%=basePath%>/product/searchBy?pageIndex=1">1</a>
	       						<a href="<%=basePath%>/product/searchBy?pageIndex=2">2</a>
	       						...
	       					...
	       					<c:otherwise>
	       						<c:if test="${(page.currentPage+1)>5}">
	       							<c:if test="${(page.currentPage+1)<=(page.totalPage-5)}">
			       						<a href="<%=basePath%>/product/searchBy?pageIndex=${page.currentPage}">&lt;Prev</a>
			       						<a href="<%=basePath%>/product/searchBy?pageIndex=${(page.currentPage+1)-4}">${(page.currentPage+1)-4}</a>
			       						<a href="<%=basePath%>/product/searchBy?pageIndex=${(page.currentPage+1)-3}">${(page.currentPage+1)-3}</a>
			       						<a href="<%=basePath%>/product/searchBy?pageIndex=${(page.currentPage+1)-2}">${(page.currentPage+1)-2}</a>
			       						<a href="<%=basePath%>/product/searchBy?pageIndex=${(page.currentPage+1)-1}">${(page.currentPage+1)-1}</a>

Ext.onReady(function(){
	...
	var btnDeclined = new Ext.Button({text:"拒绝",iconCls:'close',handler:onDeclined }) ;
	var btnAudit = new Ext.Button({text:"审核通过",iconCls:'apply',handler:onAudit }) ;
	
	new Ext.Viewport({
		layout:"border",
		items:[{
			region:"center",
			tbar:[btnSearch,"-",btnOrderDetail,"-",btnDeclined,"-",btnAudit],
			layout:"border",
			//bodyBorder:false,
			items:[
			       queryForm,
	...
	function onOrderDetail() {
	    ...
	    function onDeclined(){
			...
			var record = gridPanel.getSelectionModel().getSelected();
			if (records.length == 1) {
				declinedWin.setTitle("拒绝退款") ;
		    	declinedWin.show() ;
		    	declinedForm.getForm().reset();
		    	declinedForm.getForm().findField("id").setValue(record.data.id) ;
		...
		var declinedForm = new Ext.form.FormPanel({
			region : "center",
			frame : true,
			fileUpload : false,
			method : "POST",
		...
		var declinedWin = new Ext.Window({
			autoHeight:true,
			constrainHeader : true,
			width : 700,
			closeAction : "hide",
			...
			plain : true,
			modal : true,
			tbar : [new Ext.Button({text : "保存",iconCls:'save',handler : onDeclinedSave})],
			items :  [declinedForm]
		});
		
		function onDeclinedSave(){
			if (declinedForm.getForm().isValid() == false) {
				Ext.MessageBox.alert("提示","数据校验错误！");
				return;
			}
			
			declinedForm.getForm().submit(
							{
								url:"declined",
								waitMsg : "正在执行操作...",
								waitTitle : "请稍后...",
		...
		function onAudit(){
			...
			var record = gridPanel.getSelectionModel().getSelected();
			if (records.length == 1) {
				Ext.MessageBox.confirm('提示','是否同意退款？', function(result){
					if (result == 'yes') {
